...
- 标签：一个 Skill 可有多个标签；支持创建、重命名、删除标签；批量可追加 / 移除 / 清空。
- 分组：一个 Skill 最多属于一个分组；支持创建、重命名、排序、删除分组。
- 删除标签/分组时，只清除 Skill 上的引用，不删除 Skill 内容。
- 标签与分组只存在应用索引，不写回 `SKILL.md`（与 frontmatter `tags` 无关）。
- 左栏筛选为 AND 组合：`分组范围`（全部 / 未分组 / 某一分组）+ 多个标签（全部命中）+ 可选「无标签」。
- 中栏搜索覆盖名称、描述、来源、分组名、标签名。
- 智能分组可建议标签；可选允许新建分组/标签，确认后创建并应用。
- 本机已安装列表可只读关联库内分组/标签；taxonomy 激活时仅显示已关联库的安装。
- 空态可一键应用推荐分组/标签模板。
